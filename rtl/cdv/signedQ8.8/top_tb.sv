...
module top_tb;
        ...
        for (int r = 0; r < active_size; r++)
            for (int c = 0; c < active_size; c++)
                if (en && scoreboard_matrix[r][c] !== '0) begin
                    if ($signed(array_outputs[r][c]) === scoreboard_matrix[r][c])
                        match_count++;
                    else begin
                        mismatch_count++;
                        $error("[SIGNED Q8.8 MISMATCH] PE[%0d][%0d] Expected=%0h  Got=%0h",
    ...
    initial begin
        ...
        $display("\n[INIT] Signed Q8.8 Systolic Array - Active Size: %0d x %0d",
        ...
        drive_directed(16'h7FFF, 16'h7FFF, 1'b0);  // max_pos× max_pos, clr=accumulated
        drive_directed(16'h8000, 16'h8000, 1'b1);  // max_neg× max_neg, clr=cleared
        drive_directed(16'h4000, 16'h4000, 1'b0);  // positive×positive,clr=accumulated
        drive_directed(16'hC000, 16'hC000, 1'b1);  // negative×negative,clr=cleared
        drive_directed(16'h7FFF, 16'h8000, 1'b0);  // max_pos × max_neg (cross corner)
