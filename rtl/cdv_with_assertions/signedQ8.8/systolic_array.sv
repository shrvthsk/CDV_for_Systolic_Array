...
module systolic_array #(
    ...
    a_rst_clears_all_outputs: assert property (p_rst_clears_all_outputs)
        else $error("[ASSERT FAIL] a_rst_clears_all_outputs [Signed Q8.8]: array_out not cleared after rst");
    ...
    generate
        for (ar = 0; ar < ARRAY_SIZE; ar++) begin : assert_row
            for (ac = 0; ac < ARRAY_SIZE; ac++) begin : assert_col
                ...
                ) else $error("[ASSERT FAIL] a_no_x_out [Signed Q8.8]: X/Z on array_out[%0d][%0d]", ar, ac);
            end
        end
    endgenerate
    ...
    generate
        for (bi = 0; bi < ARRAY_SIZE; bi++) begin : assert_boundary
            ...
            ) else $error("[ASSERT FAIL] a_no_x_west [Signed Q8.8]: X/Z on west_in[%0d]", bi);

            a_no_x_north: assert property (
                @(posedge clk) disable iff (rst)
                !$isunknown(north_in[bi])
            ) else $error("[ASSERT FAIL] a_no_x_north [Signed Q8.8]: X/Z on north_in[%0d]", bi);
        end
    endgenerate
